...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		int lastElement=tokens.size()-1;
		//System.out.println(lastElement);
		if (lastElement!=-1) { //meaning if the document is not empty
			if (isWord(tokens.get(lastElement))) { //if the last token is a word, meaning there is not punctuation to signal end of a sentence
				numSentences++;
			}
		}
